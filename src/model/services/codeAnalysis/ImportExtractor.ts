...
export class ImportExtractor {
  ...
  private static extractImportPathFromNode(node: any, language: string): string | null {
    ...
    if (lang === 'typescript' || lang === 'javascript' || lang === 'tsx') {
      ...
      if (node.type === 'import_statement' || node.type === 'import_from_statement') {
        ...
        if (node.type === 'import_from_statement') {
          const moduleName = node.children.find((c: any) => c.type === 'dotted_name' || c.type === 'relative_import');
          if (moduleName) {
      ...
      if (node.type === 'import_spec') {
        const path = node.children.find((c: any) => c.type === 'interpreted_string_literal' || c.type === 'raw_string_literal');
        if (path) {
      ...
      if (node.type === 'preproc_include') {
        const path = node.children.find((c: any) => c.type === 'string_literal' || c.type === 'system_lib_string');
        if (path) {
          return this.cleanStringLiteral(path.text);
        }
      }
  ...
  // @step: [返回] 返回清理后的字符串
  private static cleanStringLiteral(str: string): string {
  ...
  // @step: [解析路径] 解析相对路径为绝对路径
  // @step: [返回] 返回可能的文件路径数组
  private static resolveImportPath(importPath: string, workspaceRoot: string, language: string): string[] {
    const path = require('path');
    const lang = language.toLowerCase();
    const files: string[] = [];
