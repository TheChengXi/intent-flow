 ...
 * 
 * 存在根因：编译器词法层规定块注释从左往右流式扫描、遇到第一个终止序列（星号斜杠）即闭合，这是语言规范不可绕开；手写代码不会触发，但代码生成器注入外部文本必然踩中（业界先例：Prisma、FlatBuffers、GraphQL Codegen 同款问题，标准解法均为转义）。转义是保持块注释前提下唯一自动方案。
 * 
 * 转义目的：终止序列提前闭合注释导致语法错误，行首 @ 被提取器误判为标签导致意图文本截断。
 * 
 * 边界：
 * - escapeBlockCommentText：终止序列全量替换为星号反斜杠斜杠；行首 @ 替换为反斜杠 @（多行模式）
