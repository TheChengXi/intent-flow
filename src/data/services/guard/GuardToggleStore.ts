/**
 * @intent
 * 守卫开关状态的持久化存储。读写项目根目录 .cdd/guard-state.json（内容 { enabled: boolean }），enabled=true 表示守卫开启（拦截确认），false 表示守卫关闭（放行）。
 ...
 * - 配置文件路径固定为 <process.cwd()>/.cdd/guard-state.json
 */
...
export class GuardToggleStore {
  private get configPath(): string {
    return join(process.cwd(), '.cdd', 'guard-state.json');
  }
  ...
  async write(enabled: boolean): Promise<void> {
    // @step 确保 .cdd 目录存在（recursive），再异步写文件；失败直接抛出，不做兜底
    await mkdir(join(process.cwd(), '.cdd'), { recursive: true });
