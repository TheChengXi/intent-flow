...
/**
 * @intent
 * 封装 ProjectIntentUseCase 为 MCP 工具，对外暴露 project_intent。自动创建父目录并根据后缀选择注释语法。force=true 时在已有文件中替换/插入 @intent，不覆盖其他内容。
 * 验收条件：
 * - inputSchema 为 zod schema：path(必填 string)、intent(必填 string)、force(可选 boolean)
 * - 执行委托 UseCase 不变
 */
...
export class ProjectIntentTool implements MCPToolHandler<ProjectIntentInput, ProjectIntentResult> {
  definition: MCPToolDefinition = {
    ...
    inputSchema: z.object({
      path: z.string().describe('目标文件路径（绝对或相对路径）'),
      intent: z.string().describe('@intent 正文内容（纯文本，工具自动添加 @intent 前缀和注释符号）'),
      force: z.boolean().optional().describe('文件已存在时是否替换/插入 @intent（默认 false，跳过不修改）')
    })
  };
