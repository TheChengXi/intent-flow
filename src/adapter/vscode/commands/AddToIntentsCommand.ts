 ...
 * 自动写入 cdd.intents.roots 设置并触发全量重建。
 */

import * as vscode from 'vscode';
import { VSCodeDIContainer } from '../VSCodeDIContainer';

export const command = 'cdd.addToIntents';
...
export async function handler(uri: vscode.Uri): Promise<void> {
  ...
  if (current.includes(relPath)) {
    vscode.window.showInformationMessage(`CDD: "${relPath}" 已在意图目录中`);
    return;
  }
  ...
  vscode.window.withProgress(
    ...
    async () => {
      try {
        ...
        const msg = `CDD: "${relPath}" 已添加 — ${result.filesCreated} 创建, ${result.filesUpdated} 更新, ${result.filesDeleted} 删除`;
        vscode.window.showInformationMessage(msg);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        vscode.window.showErrorMessage(`CDD: 扫描失败 — ${msg}`);
      }
    }
  );
}
