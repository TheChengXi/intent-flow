...
import { ReviewReport, ReviewDimension, Inconsistency, ReviewConclusion } from '../../model/entities/ReviewReport';
...
// @entity: ArbitrationChoice
// 裁决选择
export type ArbitrationChoice = 'PATH_A' | 'PATH_B' | 'CANCEL';

export class ReviewerVM extends BaseRole {
  ...
  async execute(context: ReviewContext): Promise<RoleResult> {
    try {
  ...
  async triggerArbitration(inconsistencies: Inconsistency[]): Promise<ArbitrationChoice> {
  ...
  private buildReviewDimensions(): string[] {
    return [
  ...
  private parseDimensions(content: string, dimensionNames: string[]): ReviewDimension[] {
    const dimensions: ReviewDimension[] = dimensionNames.map(name => ({
      name,
    ...
    for (const line of lines) {
      ...
      for (let i = 0; i < dimensionNames.length; i++) {
        ...
        if (match) {
          const status = match[1].toUpperCase() as 'PASS' | 'WARN' | 'FAIL';
  ...
  private determineConclusion(inconsistencies: Inconsistency[]): ReviewConclusion {
    ...
    const hasMajor = inconsistencies.some(i =>
      i.type === 'CONTRACT_MISMATCH' || i.type === 'STEP_MISSING'
    );
