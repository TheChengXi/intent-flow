...
export async function execute(): Promise<void> {
  ...
  const targetLanguage = vscode.workspace.getConfiguration('cdd').get<string>('targetLanguage') || undefined;
  ...
  if (result.success) {
    if (result.reviewPassed) {
      vscode.window.showInformationMessage(`✅ ${result.message}（重试 ${result.retryCount || 0} 次）`);
    } else {
