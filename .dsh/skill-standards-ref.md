...
- 论证说服：研究表明，问清楚再动手是最有效的系统级干预。理由：模型不需要被研究结论说服。
- 目的解释：这个规则的目的是避免蒙眼走到黑。理由：规则本身足够，目的说明是噪声。
- 比喻修辞：大问题直接写代码 ≈ 蒙眼走路。理由：比喻对模型无意义。
- 铺垫：在开始之前，让我们先确认一下。理由：直接说「确认」。
- 弱化词：你可以考虑使用、建议你。理由：直接说「用」。
- 「请」：请确认、请执行。理由：直接说「确认」「执行」。
...
常识性列举是双向盲盒：协作者不知道 → 意图不存在，写出来无意义；agent 知道 → 对话中自然知晓，写了冗余。

案例：requirement skill 曾列举实现取向清单（事件驱动、配置驱动、管道式等）。这些范式已存在于模型训练知识中，清单是在给不存在的"不知道"兜底，已删除。
...
skill 的 md 不画图、不以表格形式呈现内容。表格是服务于人眼阅读的 UI 呈现形式，skill 面向 LLM 执行，表格是画蛇添足。结构化信息直接以文本行输出，XML 式标记可选。
...
表格的 UI 元素（分隔行、对齐、管道符）在 token 流中是纯噪声。模型输出表格是预训练语料的模仿惯性——预训练需要认识表格，写给模型的内容不需要表格。

> **验证**：本文档即示例——全文无表格、无图形，所有结构化信息以文本行呈现。
...
0 前置默认：声明规则优先级（研究：Mattpocock predictability）
1 去人称化：不用「你」「用户」（研究：The Pronoun Problem 2025）
2 过程式声明：条件→动作→完成标志（研究：Prompting as Code / SPDD）
3 极简无歧义：每个词都是参数（研究：Semantic Density Effect）
4 不角色扮演：不给模型做人设（研究：PRISM / DeepMind 2025）
5 不做冗余：不解释、不论证、不铺垫（研究：FrugalPrompt）
6 不兜底：不存在的不防御；常识性列举即兜底（研究：Negation is harmful）
7 流程描述衔接：独立可读、列表连贯（研究：同流程多 skill description 编排，需可控顺序）
8 不用表格、不画图：表格 UI 服务于人，skill 不需要（研究：本文档即示例）
