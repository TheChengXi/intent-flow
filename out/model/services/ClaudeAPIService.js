...
const prompts_1 = require("../../generated/prompts");
class ClaudeAPIService {
    ...
    async callAPI(request, apiKey, baseURL, modelId, provider) {
    ...
    // @step: [返回] 返回标准响应格式
    async callAnthropic(userMessage, systemPrompt, apiKey, baseURL, modelId) {
        ...
        const response = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Errors_1.TimeoutError('API 调用超时（60s）')), 60000))
        ]);
    ...
    async callOpenAI(userMessage, systemPrompt, apiKey, baseURL, modelId) {
        const clientConfig = { apiKey };
        if (baseURL) {
            clientConfig.baseURL = baseURL;
        }
