...
exports.extractIntentWithDependencies = extractIntentWithDependencies;
exports.checkDependencyBranchSize = checkDependencyBranchSize;
...
async function extractIntentWithTreeSitter(content, language, maxLines) {
    try {
        // 获取已初始化的 parser
...
function findIntentInComments(node, maxOffset) {
    // 如果节点超出范围，停止
...
// @step: [返回] 返回 @intent 内容或 null
function extractIntentWithRegex(content, maxLines) {
    const lines = content.split('\n').slice(0, maxLines);
    // 正则匹配 @intent
    // 支持格式：
    // // @intent: 这是意图
...
async function extractIntentWithDependencies(filePath, workspaceRoot, depth = 2, visited = new Set()) {
    // 提取当前文件的意图
    ...
    try {
        ...
        const language = languageMap[ext];
        // 提取 import 的文件路径
        const importedFiles = await ImportExtractor_1.ImportExtractor.extractImportedFiles(content, path.dirname(filePath), language);
        // 递归提取每个依赖的意图
        for (const importedFile of importedFiles) {
            try {
                ...
                // 递归提取依赖
                const depBranch = await extractIntentWithDependencies(importedFile, workspaceRoot, depth - 1, visited);
...
async function checkDependencyBranchSize(branch, threshold = FileMetricsService_1.FileMetricsService.WARNING_THRESHOLD) {
    const results = [];
    try {
        // 检查当前文件
        const lineCount = await FileMetricsService_1.FileMetricsService.getLineCount(branch.filePath);
