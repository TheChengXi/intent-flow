...
class CallGraphService {
    ...
    // @end
    static async buildFileCallGraph(filePath, language) {
        // 检查缓存
    ...
    static async extractFunctions(code, language) {
        try {
            await this.init();
    ...
    // @step: [递归遍历] 递归遍历所有子节点
    static traverseForFunctions(node, functions) {
        const functionTypes = [
    ...
    static async loadLanguage(language) {
        ...
        try {
            ...
            return await web_tree_sitter_1.Language.load(wasmPath);
        }
        catch (error) {
    ...
    // @step: [返回] 返回函数名或 null
    static extractFunctionNameFromNode(node) {
        for (const child of node.children) {
    ...
    // @step: [返回] 返回映射
    static async extractCalls(code, language) {
        const calls = new Map();
        try {
            await this.init();
            const lang = await this.loadLanguage(language);
    ...
    // @step: [递归遍历] 递归遍历所有子节点
    static traverseForCalls(node, calls) {
        const functionTypes = [
            'function_declaration',
            'function_definition',
            'method_declaration',
    ...
    // @step: [递归遍历] 递归遍历所有子节点
    static findCallsInNode(node, callees) {
        const callTypes = [
            'call_expression',
            'call'
        ];
    ...
    // @step: [返回] 返回函数名或 null
    static extractCalleeNameFromNode(node) {
    ...
    static getCallers(functionName, graph) {
    ...
    // @boundary: 当函数不存在时，返回空数组
    static getCallees(functionName, graph) {
        const node = graph.nodes.get(functionName);
    ...
    static collectDependencies(functionName, graph, depth = 3) {
        const visited = new Set();
        const result = new Set();
        const dfs = (name, currentDepth) => {
...
// 内存缓存
CallGraphService.cache = new Map();
CallGraphService.parser = null;
CallGraphService.initialized = false;
//# sourceMappingURL=CallGraphService.js.map