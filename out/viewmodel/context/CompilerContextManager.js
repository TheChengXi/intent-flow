...
class CompilerContextManager {
        ...
        try {
            ...
            if (!hasValidPreviousCode) {
                ...
                if (filteredCalls.length > 0) {
                    ...
                    const functionContracts = await WorkLineService_1.WorkLineService.searchContractsForFunctions(filteredCalls, searchFiles, workspaceRoot);
                    ...
                    for (const funcName of filteredCalls) {
                        ...
                        for (const filePath of searchFiles) {
                            const contracts = await HistoryService_1.HistoryService.getAllContractsForFunction(workspaceRoot, filePath, funcName);
    ...
    static async selectCompileSpec(workspaceRoot, filePath) {
    ...
    // @boundary: 当所有规范都不存在时，返回空字符串
    static async selectByPath(workspaceRoot, filePath) {
        const path = require('path');
    ...
    static async selectByConfig(workspaceRoot, filePath, rules) {
        const path = require('path');
    ...
    static extractExistingImports(fileContent, language) {
        const imports = [];
        // 根据语言选择不同的正则表达式
        let importRegex;
        if (language) {
