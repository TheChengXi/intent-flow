...
const TranslatorContextManager_1 = require("../context/TranslatorContextManager");
...
async function execute() {
    ...
    // 准备上下文
    const context = await TranslatorContextManager_1.TranslatorContextManager.prepare(editor.document, selection, workspaceRoot, apiKey, apiBaseUrl, modelId);
    ...
    // 保存历史
    await TranslatorContextManager_1.TranslatorContextManager.save(workspaceRoot, editor.document.fileName, functionName, functionName + ':v1.0', context.code, commentText, true, context.compileSpec);
    const now = new Date();
