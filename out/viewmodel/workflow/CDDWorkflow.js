...
exports.executeCDDWorkflow = executeCDDWorkflow;
...
const CompilerContextManager_1 = require("../context/CompilerContextManager");
const ReviewerContextManager_1 = require("../context/ReviewerContextManager");
...
async function executeCDDWorkflow(context) {
    const executionPath = [];
    const MAX_RETRY = 3;
    try {
        // WF-01: 解析输入
...
function detectWorkflowType(selectedText, document, startLine) {
...
// @step: [返回结果] 返回最终结果
// @boundary: 当达到最大重试次数时，提示用户选择路径A或路径B
async function executeCompileWorkflow(context, executionPath, maxRetry) {
    ...
    while (retryCount < maxRetry) {
        ...
        if (retryCount > 0 && lastCodeInsertPosition) {
            ...
            if (editor) {
                await editor.edit(editBuilder => {
                    const endPosition = editor.document.positionAt(editor.document.offsetAt(lastCodeInsertPosition) + lastCode.length + 2);
                    editBuilder.delete(new vscode.Range(lastCodeInsertPosition, endPosition));
                });
            }
        }
        console.log('[CDDWorkflow] 准备编译上下文...');
        const compileContext = await CompilerContextManager_1.CompilerContextManager.prepare(context.document, context.selection, context.workspaceRoot, context.apiKey, context.apiBaseUrl, context.modelId, context.targetLanguage);
        if (!compileContext) {
        ...
        await CompilerContextManager_1.CompilerContextManager.save(context.workspaceRoot, context.document.fileName, compileContext.comment.contract.functionName, `${compileContext.comment.contract.functionName}:v1.0`, context.document.getText(context.selection), lastCode, true, compileContext.compileSpec);
        ...
        const reviewContext = await ReviewerContextManager_1.ReviewerContextManager.prepare(context.document, context.selection, context.workspaceRoot, context.apiKey, context.apiBaseUrl, context.modelId);
        if (!reviewContext) {
        ...
        if (reviewResult.artifacts && typeof reviewResult.artifacts === 'object' && 'inconsistencies' in reviewResult.artifacts) {
            const report = reviewResult.artifacts;
            console.log('[CDDWorkflow] 审查报告:', JSON.stringify(report, null, 2));
            if (Array.isArray(report.inconsistencies)) {
                issues.push(...report.inconsistencies.map((inc) => `${inc.type}: ${inc.description}`));
            }
        }
        console.log('[CDDWorkflow] 提取到的问题数量:', issues.length);
        console.log('[CDDWorkflow] 保存审查结果到历史记录...');
        await ReviewerContextManager_1.ReviewerContextManager.save(context.workspaceRoot, context.document.fileName, reviewContext.comment.contract.functionName, `${reviewContext.comment.contract.functionName}:v1.0`, reviewContext.comment, lastCode, reviewResult.success, issues, reviewContext.compileSpec);
        ...
        if (retryCount >= maxRetry) {
            ...
            const choice = await vscode.window.showWarningMessage(`经过 ${maxRetry} 次编译，代码仍未通过审查。\n\n路径A（注释为准）：删除代码，修改注释后重新编译\n路径B（代码为准）：保留代码，执行"转译代码为注释"`, '路径A：重新编译', '路径B：反向同步', '取消');
        ...
        const continueChoice = await vscode.window.showWarningMessage(`审查未通过（第 ${retryCount} 次）。是否根据审查反馈重新编译？`, '重新编译', '停止（保留当前代码）');
...
// @step: [返回结果] 返回审查结果
async function executeReviewWorkflow(context, executionPath) {
    executionPath.push('reviewer');
    const apiService = new ClaudeAPIService_1.ClaudeAPIService();
    const reviewerVM = new ReviewerVM_1.ReviewerVM(apiService);
    const reviewContext = await ReviewerContextManager_1.ReviewerContextManager.prepare(context.document, context.selection, context.workspaceRoot, context.apiKey, context.apiBaseUrl, context.modelId);
...
// @end
//# sourceMappingURL=CDDWorkflow.js.map