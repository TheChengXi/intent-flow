...
const CallGraphService_1 = require("../../model/services/CallGraphService");
...
class CompilerVM extends BaseRole_1.BaseRole {
    ...
    async execute(context) {
        try {
            ...
            // 构建用户消息（函数调用风格）
            const userMessage = this.buildUserMessage(commentText, language, referencedContractsText, reviewFeedbackText, incrementalText);
            const request = {
                role: 'compiler',
            ...
            const referencedContractNames = new Set((context.referencedContracts || []).map(contract => {
    ...
    // @step: [返回] 返回依赖数组
    async extractDependencies(code, filePath, language) {
        const dependencies = [];
        const seen = new Set();
        // 标准库函数列表（问题 4 修复）
        ...
        if (filePath && language) {
            try {
                ...
                const graph = await CallGraphService_1.CallGraphService.buildFileCallGraph(tempFile, language);
                // 清理临时文件
    ...
    // @step: [返回] 返回完整的用户消息
    buildUserMessage(commentText, language, referencedContracts, reviewFeedback, incrementalText) {
        let message = '';
        // 必需参数
        message += `comment:\n${commentText}\n\n`;
