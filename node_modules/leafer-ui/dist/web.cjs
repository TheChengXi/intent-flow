...
function checkSizeAndCreateData(ui, attrName, paint, image, leafPaint, boxBounds) {
    ...
    if (paint.mode === "brush") draw.PaintImage.brush(ui, attrName, leafPaint);
    if (!leafPaint.data) {
...
function createPattern(paint, ui, canvas, renderOptions) {
    ...
    if (paint.patternId !== id && !ui.destroyed) {
        if (!(core.Platform.image.isLarge(paint.image, scaleX, scaleY) && !paint.data.repeat)) {
            const {image: image, brush: brush, data: data} = paint, {transform: transform, gap: gap} = data, fixScale = draw.PaintImage.getPatternFixScale(paint, scaleX, scaleY);
            ...
            if (brush) {
                ...
                draw.PaintImage.cacheBrush(paint, ui, canvas, renderOptions);
            } else {
                paint.style = image.getPattern(imageCanvas, data.repeat || (core.Platform.origin.noRepeat || "no-repeat"), imageMatrix, paint);
            }
            paint.patternId = id;
        }
    }
...
function checkImage(paint, drawImage, ui, canvas, renderOptions) {
    ...
    if (!data || paint.patternId === id && !exporting || snapshot) {
        ...
        if (drawImage) {
            if (data.repeat) {
                drawImage = false;
            } else if (!(originPaint.changeful || paint.film || core.Platform.name === "miniapp" || exporting)) {
