...
import { CDDComment, StepAnnotation } from '../entities/CDDComment';

// @entity: StepDiff
// 步骤差异结果
export interface StepDiff {
  unchanged: StepAnnotation[];      // 未变化的步骤
  added: StepAnnotation[];          // 新增的步骤
  modified: StepAnnotation[];       // 修改的步骤
  deleted: StepAnnotation[];        // 删除的步骤
  unchangedRatio: number; // 未变化步骤占比（0-1）
}
...
export function detectDiff(oldComment: CDDComment, newComment: CDDComment): StepDiff {
  ...
  const unchanged: StepAnnotation[] = [];
  const added: StepAnnotation[] = [];
  const modified: StepAnnotation[] = [];
  const deleted: StepAnnotation[] = [];

  // 构建旧哈希映射（哈希 -> 步骤列表，因为可能有重复哈希）
  const oldHashMap = new Map<string, StepAnnotation[]>();
  ...
  // 匹配新步骤
  const matchedOldSteps = new Set<StepAnnotation>();
...
export function computeStepHash(step: StepAnnotation): string {
...
export function shouldUseIncrementalMode(diff: StepDiff, threshold: number = 0.5): boolean {
